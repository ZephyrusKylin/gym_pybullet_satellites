...
def print_evaluation_details(sim_type, strategy_vector, final_score, final_score_text):
    ...
    if sim_type=="演进前":print("  - Agent-1: 最终拦截机动 -> 失败 (代价过高)")
    else:print("  - Agent-1/2/3: 防区外打击阵位机动 -> 成功")
...
def update(frame):
    ...
    if frame == 0:
        ...
        # 重置所有文本
        status_texts['A1'].set_text('我方 Agent-1: ...');status_texts['A1'].set_color('white')
        status_texts['A2'].set_text('我方 Agent-2: ...');status_texts['A2'].set_color('white')
        status_texts['A3'].set_text('我方 Agent-3: ...');status_texts['A3'].set_color('white')
    ...
    if frame < PRE_SIM_END_FRAME:
        ...
        if not agent1_destroyed:status_texts['A1'].set_text('我方 Agent-1: 快速拦截中...')
        status_texts['A2'].set_text('我方 Agent-2: 支援机动中...');status_texts['A3'].set_text('我方 Agent-3: 支援机动中...');
        ...
        if f >= PRE_EVO_LOSS_FRAME_LOCAL and not agent1_destroyed:
            agent1_destroyed=True;sats_f1.set_color(COLOR_DESTROYED);status_texts['A1'].set_text('我方 Agent-1: 已被击毁');status_texts['A1'].set_color(COLOR_DESTROYED);status_texts['loss'].set_text('我方损失: 1')
    ...
    # 阶段三: 演进后作战重现
        ...
        if f == 1: 
            ...
            status_texts['A1'].set_text('我方 Agent-1: ...');status_texts['A1'].set_color('white');status_texts['loss'].set_text('我方损失: 0');status_texts['E'].set_text('敌方目标: 存活');status_texts['E'].set_color('white');
        ...
        status_texts['A1'].set_text('我方 Agent-1: 安全协同机动...');status_texts['A2'].set_text('我方 Agent-2: 安全协同机动...');status_texts['A3'].set_text('我方 Agent-3: 安全协同机动...');
