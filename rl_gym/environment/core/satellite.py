...
class Satellite:
    """代表单个卫星的物理状态和属性 (Poliastro 版本)。"""
    def __init__(self, sat_id: str, initial_orbit: Orbit, mass_wet: u.Quantity, 
                 dry_mass: u.Quantity, isp: u.Quantity, attributes: Dict[str, Any] = None):
        self.sat_id = sat_id
        self.orbit = initial_orbit
        self.mass = mass_wet.to(u.kg)
        self.dry_mass = dry_mass.to(u.kg)
        self.isp = isp.to(u.s)
        self.attributes = attributes if attributes is not None else {}
    ...
    def __repr__(self):
        return (f"<Satellite id='{self.sat_id}' mass={self.mass:.2f}, fuel={self.fuel_mass:.2f}>")

    @property
    def r(self): return self.orbit.r
    @property
    def v(self): return self.orbit.v
    @property
    def epoch(self): return self.orbit.epoch
    @property
    def fuel_mass(self): return self.mass - self.dry_mass
    @property
    def can_maneuver(self): return self.mass > self.dry_mass
    ...
    @staticmethod
    def from_classical(
        sat_id: str, a: u.Quantity, ecc: u.Quantity, inc: u.Quantity,
        raan: u.Quantity, argp: u.Quantity, nu: u.Quantity,
        mass_wet: u.Quantity, dry_mass: u.Quantity, isp: u.Quantity,
        epoch: Time, mu, attributes: Dict[str, Any] = None
    ) -> 'Satellite':
        """
        通过经典的开普勒轨道六根数创建一个 Satellite 实例。
        这是一个便捷的工厂方法。
        """
        from poliastro.bodies import Earth  # 局部导入以避免循环依赖

        orbit = Orbit.from_classical(
            attractor=Earth, a=a, ecc=ecc, inc=inc, 
            raan=raan, argp=argp, nu=nu, epoch=epoch
        )
        # 手动设置引力常数，因为 from_classical 默认使用 Earth.k
        # 这确保了与我们 constants.py 中 mu 的一致性
        orbit._k = mu 

        return Satellite(
            sat_id=sat_id, initial_orbit=orbit, mass_wet=mass_wet, 
            dry_mass=dry_mass, isp=isp, attributes=attributes
        )